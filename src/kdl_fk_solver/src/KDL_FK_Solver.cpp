...
KDL::ChainFkSolverPos_recursive KDL_FK_Solver::setup_right_kdl()
{
  ...
  const std::string JOINT2_LINK = "yumi_link_2_r";
  ...
  // Find segment number for wrist and elbow links, store in constraint_dat
  if (r_num_wrist_seg == 0 || r_num_elbow_seg == 0 || r_num_shoulder_seg == 0|| r_num_joint2_seg == 0) // if the IDs not set
  {
    unsigned int num_segments = kdl_chain.getNrOfSegments();
    ...
    for (unsigned int i = 0; i < num_segments; ++i){
      ...
      if (kdl_chain.getSegment(i).getName() == JOINT2_LINK){
        r_num_joint2_seg = i;
        //ROS_INFO_STREAM("Elbow link found.");
        //break;
      }
...
/* Return left or right shoulder orientation (rotation matrix) */
Matrix3d KDL_FK_Solver::return_shoulder_ori(KDL::ChainFkSolverPos_recursive &fk_solver, Matrix<double, 7, 1> q_cur, bool left_or_right)
{
      // Get joint angles
      KDL::JntArray q_in(q_cur.size()); 
      for (unsigned int i = 0; i < q_cur.size(); ++i)
      ...
      // Do FK using KDL, get the current elbow/shoulder/shoulder state
      KDL::Frame shoulder_cart_out; // Output homogeneous transformation
      int result;
      if(left_or_right) // left arm
      {
        result = fk_solver.JntToCart(q_in, shoulder_cart_out, l_num_shoulder_seg+1);
      ...
      if (result < 0){
        ...
        //ROS_INFO_STREAM("FK solver succeeded for shoulder link.");
      }

      // Preparations
      Matrix3d shoulder_ori_cur = Map<Matrix<double, 3, 3, RowMajor> >(shoulder_cart_out.M.data, 3, 3); 

  // Return cost function value
  return shoulder_ori_cur;

}

/* Return position */
Vector3d KDL_FK_Solver::return_pos(KDL::ChainFkSolverPos_recursive &fk_solver, Matrix<double, 7, 1> q_cur, unsigned int ind)
{
  // Get joint angles
  KDL::JntArray q_in(q_cur.size()); 
  for (unsigned int i = 0; i < q_cur.size(); ++i)
  ...
  // Do FK using KDL, get the current elbow/wrist/shoulder state
  KDL::Frame cart_out;//, shoulder_cart_out; // Output homogeneous transformation
  int result;
  result = fk_solver.JntToCart(q_in, cart_out, ind+1);
  if (result < 0){
    //ROS_INFO_STREAM("FK solver failed when processing wrist link, something went wrong");
    exit(-1);
  }
  ...
  // Preparations
  Vector3d pos_cur = Map<Vector3d>(cart_out.p.data, 3, 1);
...
/* Return left or right shoulder orientation (rotation matrix) */
Matrix3d KDL_FK_Solver::return_ori(KDL::ChainFkSolverPos_recursive &fk_solver, Matrix<double, 7, 1> q_cur, unsigned int ind)
{
      // Get joint angles
      KDL::JntArray q_in(q_cur.size()); 
      for (unsigned int i = 0; i < q_cur.size(); ++i)
      ...
      // Preparations
      Matrix3d ori_cur = Map<Matrix<double, 3, 3, RowMajor> >(cart_out.M.data, 3, 3); 
...
int main(int argc, char **argv)
{
  ...
  std::cout << "l_wrist_pos= " << l_wrist_pos.transpose() 
            ...
            << "\n, l_elbow_quat(w,x,y,z)= " << l_elbow_quat.w() << " "
                                             << l_elbow_quat.x() << " "
                                             << l_elbow_quat.y() << " "
                                             << l_elbow_quat.z() 
  ...
  std::cout << "r_wrist_pos= " << r_wrist_pos.transpose() 
            ...
            << "\n, r_shoulder_pos= " << r_shoulder_pos.transpose() 
             << "\n, r_joint2_pos= " << r_joint2_pos.transpose() 
            << "\n, r_wrist_quat(w,x,y,z)= " << r_wrist_quat.w() << " "
                                             << r_wrist_quat.x() << " "
                                             << r_wrist_quat.y() << " "
                                             << r_wrist_quat.z() << std::endl;
  std::cout << "r_wrist_pos_y= " << r_wrist_pos.y() << std::endl;

  std::cout << "r shoulder rot =\n" << r_shoulder_rot << std::endl;
  std::cout << "r join2 rot =\n" << r_joint2_rot << std::endl; 
  std::cout << "l elbow rot =\n" << l_elbow_rot << std::endl; 
  std::cout << "r elbow rot =\n" << r_elbow_rot << std::endl; 
  std::cout << "l wrist rot =\n" << l_wrist_rot << std::endl; 
  std::cout << "r wrist rot =\n" << r_wrist_rot << std::endl; 
